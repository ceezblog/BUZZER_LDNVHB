...
#define WIN32_UDP_HELPER

#ifdef WIN32_UDP_HELPER
typedef UINT8 byte; // use "byte" to unify with arduino code
typedef UINT8 uint8_t; // use "uint8_t" to unify with arduino code
typedef UINT16 uint16_t; // use "uint16_t" to unify with arduino code
typedef UINT32 uint32_t; // use "uint32_t" to unify with arduino code
#endif
...
// M_NULL0, // NULL
// M_PING = 6, // test alive, for BZ_QDATAGRAM
// M_ACK = 7, // acknowledge the message for BZ_QDATAGRAM
// M_OK = 8, // command ok for BZ_QDATAGRAM
// M_FAIL = 9, // command fail for BZ_QDATAGRAM
// M_RESET = 10, // RESET ALL TEXT
// M_SCREENSAVER = 18,    //screen saver
// M_LAYOUT = 19,   // define layout
// M_SET_TEXT = 20,
// M_SHOW_TEXT = 30,  
// M_SHOW_INSTANT_TEXT = 40, // Host controlled message - hide current layout, show text for a few second.
